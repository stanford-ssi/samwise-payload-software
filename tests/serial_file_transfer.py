...
class SerialFileTransfer:
    ...
    def receive_file(self, local_path):
        ...
        print(f"Waiting to receive num packets...")
        ...
        print(f"Receiving group {missing_packets[0]}-{missing_packets[-1]}...")
        ...
        with open(local_path, 'wb+') as f:
            ...
            while consecutive_bad_packets < MAX_BAD_PACKETS:
                ...
                if response is None:
                    ...
                    print(f"Received a bad packet! ({consecutive_bad_packets} in a row)")
                    continue
                ...
                print(f"{missing_packets} are missing!")    

                # Special command to abort the transfer
                if chunk == ABORT_FILE_TRANSFER:
                    print(f"ABORTING FILE TRANSFER!")
                    return False
                ...
                # Special command to send an acknowledgement
                    ...
                    if missing_packets == []:
                        ...
                        if received_last_packet:
                            print(f"File transfer is COMPLETE!")
                            return True

                        group_start += GROUP_SIZE
                        missing_packets = [i for i in range(group_start, min(group_start + GROUP_SIZE, num_packets))]
                        
                        print(f"Group was successfully received!")
                        print(f"New group is packets {missing_packets[0]}-{missing_packets[-1]}")
    ...
    def send_file(self, filename):
        ...
        with open(filename, 'rb') as f:
            ...
            print(f"About to send {num_packets} packets...")
            ...
            if num_packets != num_full_packets:
                ...
                print(f"Sending packet {seq_num} (partial last packet!)...")
    ...
    def _send_missing_packets(self, f):
        ...
        for _ in range(MAX_RESEND_CYCLES):
            ...
            if missing_packets is None:
                # Receiver did not repsond - abort file transfer
                print(f"Receiver is not responsive - aborting file transfer!")
                self.packet_handler.write_packet(ABORT_FILE_TRANSFER)
                return False

            # If no packets were missing, return True
            if missing_packets == []:
                print(f"Receiver has received this group successfully!")
                return True
            ...
            for seq_num in missing_packets:
                ...
                print(f"Re-sending packet {seq_num}...")
                self.packet_handler.write_packet(
                    write_buf_view,
                    seq_num=seq_num
                )
        ...
        print(f"Packets are still missing - aborting file transfer!")
        self.packet_handler.write_packet(ABORT_FILE_TRANSFER)
        return False
    ...
    def _request_missing_packets(self):
        ...
        for _ in range(MAX_ACK_RETRIES):
            print(f"Requesting missing packets...")
            ...
            if response is not None:
                ...
                print(f"Receiver is missing packets {missing_packets}!")
                return missing_packets

            print(f"Receiver did not respond, trying again...")
